...
private val repo = DoltRepository()

/**
 * Minimal HTML CRUD frontend on top of Dolt, showing the whole workflow you asked about:
 *  - edit rows (ordinary SQL against a MySQL-compatible server),
 *  - *review* the uncommitted result as a row-level diff before anything is final,
 *  - then either commit (confirm) or discard (reset --hard).
 *
 * Run with `./gradlew runDoltWeb`, then open http://localhost:8080
 */
fun main() {
...
private fun HTML.page(flash: String? = null) {
...
private fun FlowContent.diffCell(from: String?, to: String?, diffType: String) {
    when {
