...
/**
 * Compare each method here to its DSL counterpart in [ExposedModelConfigService]: no `toRow()`
 * mapping function, no `it[column] = value` assignment blocks, no manual "update, and if 0 rows
 * were touched then insert" upsert logic - [org.jetbrains.exposed.dao.EntityClass] gives you
 * `findById`/`new`/property assignment directly, letting the entity read like a plain mutable
 * object.
 */
class ExposedDaoModelConfigService(jdbcUrl: String = "jdbc:h2:file:./data/config-exposed-dao;INIT=RUNSCRIPT FROM 'classpath:schema.sql'") {
