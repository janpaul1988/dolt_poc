 ...
 * one-line `transaction { ... }` block. Compare to [ExposedDaoModelConfigService], which uses
 * Exposed's higher-level DAO API for the same table.
 */
class ExposedModelConfigService(jdbcUrl: String = "jdbc:h2:file:./data/config-exposed;INIT=RUNSCRIPT FROM 'classpath:schema.sql'") {
    ...
    private fun ResultRow.toRow() = ModelConfigRow(
